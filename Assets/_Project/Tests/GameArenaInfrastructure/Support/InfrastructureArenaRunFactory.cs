...
namespace Game.Arena.Infrastructure.Tests.Support
{
    internal sealed class InfrastructureArenaRunFactory
    {
        private readonly MovementPathPolicy _movementPathPolicy;

        public InfrastructureArenaRunFactory()
        {
            _movementPathPolicy = new MovementPathPolicy();
        }

        public ArenaRun Create(
            ArenaRunId arenaRunId,
            PlayerId playerId)
        {
            ArenaDefinition arena = new(new ArenaBounds(-20f, 20f, -20f, 20f, 0f), Distance.FromValue(5f));
            ...
            ArenaRunFactory factory = new(_movementPathPolicy);
